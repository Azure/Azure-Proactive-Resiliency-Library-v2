...
```powershell
-tags "App||Application=~App1||App2","env||environment=~prod||production","region=~eastus"
```

- Our next example will demonstrate how we can filter using a **NOT** operator. This will return all resources in scope, except those that meet the requirements of app or application not equalling App3, env or environment not equalling dev or qa, and the region not equalling westus.
...
```powershell
-tags "App||Application!~App3","env|environment!~dev||qa","region!~westus"
```
